...
public class Red_Matter_Chestplate_Recipe {
    public static ShapedRecipe get() {//处于未知原因，貌似背包里的皮革靴子和.get()获取的不是同一个，虽然应该是同一个才对，所以合成被移到Event.Items.Armor.Armor_Craft里了
        ShapedRecipe shapedRecipe = new ShapedRecipe(new NamespacedKey(PluginE.instance,"red_matter_chestplate"), Red_Matter_Chestplate.get());
        shapedRecipe=shapedRecipe.shape("rdr",
                                        "rrr",
                                        "rrr");
        shapedRecipe.setIngredient('r', Red_Matter.get())
                    .setIngredient('d',Dark_Matter_Chestplate.get());
        return shapedRecipe;
    }
}
